...
public class AddonPickaxeItem extends PickaxeItem implements IAddonItem {
    ...
    public static class Parser implements ItemParser.ItemTypeSerializer {
        ...
        public IAddonItem parse(JsonObject json, Properties properties) {
            ...
            return new AddonPickaxeItem(tier, GsonHelper.getAsInt(json, "base_damage"), GsonHelper.getAsFloat(json, "attack_speed"), properties);
        }

        @Override
        public void generateDocumentation(JsonDocumentationBuilder builder) {
            builder.setTitle("Pickaxe");
            ...
            builder.addProperty("base_damage", Integer.class)
                    .description("Base value for the damage. For reference: pickaxes usually have 1")
                    .required().exampleJson(new JsonPrimitive(1));

            builder.addProperty("attack_speed", Float.class)
                    .description("Base value for the attack speed. For reference: pickaxes usually have -2.8")
                    .required().exampleJson(new JsonPrimitive(-2.8));
        }
